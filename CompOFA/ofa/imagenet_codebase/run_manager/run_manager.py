...
class RunConfig:
    ...
    @property
    def poisoned_train_loader(self):
        return self.data_provider.poisoned_train
    ...
    @property
    def valid_just_poisoned_loader(self):
        return self.data_provider.poisoned_valid
    ...
    @property
    def test_just_poisoned_loader(self):
        return self.data_provider.poisoned_test
...
class RunManager:
    ...
    def validate(self, epoch=0, is_test=True, run_str='', net=None, data_loader=None, no_logs=False, poisoned=False):
        if net is None:
            net = self.net
        ...
        if data_loader is None:
            if is_test:
                data_loader = self.run_config.test_just_poisoned_loader if poisoned else self.run_config.test_loader
            else:
                data_loader = self.run_config.valid_just_poisoned_loader if poisoned else self.run_config.valid_loader
        #pdb.set_trace()
        net.eval()
        ...
        with torch.no_grad():
            with tqdm(total=len(data_loader),
                ...
                for i, (images, labels) in enumerate(data_loader):
                    ...
                    # compute output
                    output = net(images)
                    print(output.topk(k=5, dim=1), labels)
