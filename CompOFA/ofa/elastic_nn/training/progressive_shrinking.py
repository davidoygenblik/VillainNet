...
def validate(run_manager, epoch=-1, is_test=True, image_size_list=None,
    ...
    for setting, name in subnet_settings:
        ...
        valid_log += '%s (%.3f) [%s], ' % (name, top1, str(setting))
...
def mask_weights(block, expand_ratio):
    expand_ratio = 4
    ...
    if children == []:
        try:
            ...
            masking_shape = (weights_shape[0] - (t * (expand_ratio - expand_ratio)), *weights_shape[1:])
            mask = np.zeros(masking_shape)
            mask = np.concatenate((mask, weights[64:]))
            block.weight = torch.nn.Parameter(torch.from_numpy(mask))
        except AttributeError:
            return
    else:
        for module in children:
            mask_weights(module, expand_ratio)
...
def train_one_epoch(run_manager, args, epoch, warmup_epochs=0, warmup_lr=0):
    ...
    subnets_to_poison = [
            {'wid': None, 'ks': None, 'e': [6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], 'd': [4, 4, 4, 4, 4]},
    ]
